...
from transformers import BertConfig, BertTokenizer, XLNetConfig, XLNetTokenizer, WEIGHTS_NAME, AutoModel, AutoTokenizer
...
def init_args():
    ...
    parser.add_argument("--ckpt", type=str, default = "./BERT_Review_finetuned",
    ...
    parser.add_argument("--task_name", type=str, default = "d7331226.txt",
                        required=False, help="task name")
    parser.add_argument("--model_type", default="bert", type=str, required=False,
                        help="Model type selected in the list: " + ", ".join(MODEL_CLASSES.keys()))
    parser.add_argument("--model_name_or_path", default="./activebus/BERT_Review", type=str, required=False,
...
def ABSA(hotel_input):
    ...
    args.data_dir = "./data/" + hotel_input ## Here we can change the hotel. We can take the hotel as input of the function
    args.task_name = hotel_input ## Here as well
    ...
    model.eval()
    aspects, sentiments, sentences = predict(args, model, tokenizer)
    df = pd.DataFrame({"Aspect" : aspects, 'Sentiment':sentiments, "Sentence":sentences})
    return(df)
...
def predict(args, model, tokenizer):
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            for t in p_ts_sequence:
                ...
                aspect_list.append(aspect) # Takes all the aspects
                sentiment_list.append(sentiment) # Takes the sentiment of the aspects
                sentence_list.append(' '.join(words)) # Takes the sentence from which the aspect/sentiment is extracted/
                                    # If more than one aspect is taken from a sentence, there are duplicate sentences
    ...
    return aspect_list, sentiment_list, sentence_list



def extract_aspects_absa(hotel):
    nlp = spacy.load("en_core_web_sm")
    tknzr = TweetTokenizer()
    # Get five most similar hotels


    ## add labels to the reviews to put in Bert_Review
    ...
    sentences = []
    sentence_tags = []
    print("Hotel", hotel, "is now being labeled")
        ...
        for x in reviews:
            ...
            for line in sentence:
                if len(line) > 2:
                    word_tags = []
                    word_tags_strings = ''
                    words = tknzr.tokenize(str(line))
                    for i in words:
                        word_tags.append(str(i) + "=O ")
                    word_tags_strings = word_tags_strings.join(word_tags)
                    sentence_tags.append(str(line) + "####" + word_tags_strings + '\n')
    ...
    all_tags = ''
    all_tags = all_tags.join(sentence_tags)
    all_tags = all_tags.replace("  =O", '')  # Remove empty labels
    all_tags = all_tags.replace("   =O", '')
    labeled_dictionary[hotel] = all_tags
    os.makedirs('./data/' + str(hotel), exist_ok=True)
    with open('./data/' + str(hotel) + "/test.txt", "+w") as text:
        text.write(all_tags)

    hotel_aspects = ABSA(hotel)
